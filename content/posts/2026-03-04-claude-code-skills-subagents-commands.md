...
```mermaid
graph LR
    Normal[일반 프롬프트\n매번 새로 작성] -->|반복 작업| Waste[맥락 손실\n비일관성]
    Skill[Skill 파일\n한 번 정의] -->|자동 트리거| Consistent[일관된 워크플로우\n재사용 가능]
```
...
```mermaid
graph TD
    Main[메인 에이전트\n전체 조율] -->|암호화 모듈 작성| Sub1[Subagent 1\n깨끗한 컨텍스트]
    Main -->|유효성 검사 로직| Sub2[Subagent 2\n깨끗한 컨텍스트]
    Main -->|테스트 작성| Sub3[Subagent 3\n깨끗한 컨텍스트]
    Sub1 -->|결과만 반환| Main
    Sub2 -->|결과만 반환| Main
    Sub3 -->|결과만 반환| Main
```
...
```mermaid
graph LR
    User[/review 입력] --> Cmd[Commands 레이어\n명령어 파싱]
    Cmd --> Skill[Skills 레이어\n워크플로우 실행]
