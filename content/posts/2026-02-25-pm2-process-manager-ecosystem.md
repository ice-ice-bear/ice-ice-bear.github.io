...
graph TD
    ...
    B --> C["backend 프로세스<br/>uv run uvicorn :8000"]
    B --> D["frontend 프로세스<br/>npm run dev :5173"]
    C -->|크래시| E[자동 재시작 autorestart]
    D -->|크래시| E
    E --> B
    F[pm2 save] --> G["~/.pm2/dump.pm2<br/>프로세스 목록 저장"]
    H[pm2 startup] --> I["시스템 서비스 등록<br/>재부팅 후 자동 복구"]
    G --> I
```
...
graph LR
    ...
    C[".env<br/>GOOGLE_API_KEY<br/>OPENAI_API_KEY"] -->|python-dotenv 로드| A
    D["ecosystem.config.js<br/>NODE_ENV=production"] -->|PM2 주입| A
    E[Nginx 또는 직접 접근] --> A
    E --> B
```
