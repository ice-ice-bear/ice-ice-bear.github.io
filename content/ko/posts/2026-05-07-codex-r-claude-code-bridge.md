...
> **업데이트 (2026-05-04 전후 정황):** 같은 주에 [Codex가 ChatGPT 안으로 들어오고](https://help.openai.com/en/articles/11369540-codex-in-chatgpt) [Codex Python SDK](https://github.com/openai/codex/tree/main/sdk/python)가 모노레포에 합류했다. 이 글의 분석은 그 변화 이후에도 유효하지만, 아래 "Codex 표면 확장" 절에서 그 함의를 따로 짚는다.
...
이 포스트를 처음 쓸 때 Codex의 사용자 진입점은 사실상 CLI 하나였다. 며칠 사이 그림이 꽤 달라졌다. OpenAI는 [Codex를 ChatGPT 안으로 끌어왔고](https://help.openai.com/en/articles/11369540-codex-in-chatgpt) — ChatGPT Plus·Pro·Business·Enterprise·Edu 플랜에 Codex 사용량이 포함되며, 한시적으로 Free·Go까지 풀린다. 진입점은 이제 **[Codex 앱](http://developers.openai.com/codex/app), [Codex CLI](https://developers.openai.com/codex/cli), [Codex IDE extension](http://developers.openai.com/codex/ide), [Codex web (chatgpt.com/codex)](https://chatgpt.com/codex)** 네 갈래로 갈라졌고, 모두 같은 ChatGPT 로그인을 공유한다. 거의 같은 시점에 [Codex Python SDK](https://github.com/openai/codex/tree/main/sdk/python)가 `openai/codex` 모노레포의 `sdk/python`으로 들어왔다 — `app-server`의 JSON-RPC v2를 stdio로 감싼 실험적 Pydantic SDK로, `with Codex() as codex: codex.thread_start(model="gpt-5")` 한 줄로 thread 생애주기를 잡는다. 패키지는 `openai-codex-app-server-sdk`와 플랫폼 휠로 배포되는 `openai-codex-cli-bin` 두 개로 쪼개졌고, **SDK 버전이 곧 Codex 런타임 버전**으로 핀된다.

이 두 변화는 codex-r 같은 bridge에 양면으로 작용한다. 한쪽에선 **bridge할 surface가 늘었다** — CLI뿐 아니라 IDE extension, 웹 UI, Python 프로세스 어디서나 Codex thread를 띄울 수 있고, [`externalAgentConfig/import`](https://github.com/openai/codex) 같은 app-server RPC가 SDK에서 1급 호출이 되면 picker를 Python 스크립트로 다시 짤 수도 있다. 다른 한쪽에선 **bridge 자체의 위치가 흔들린다** — ChatGPT 안에서 Codex가 [Memories](https://developers.openai.com/codex/memories), [Automations](https://developers.openai.com/codex/app/automations), [in-app browser](https://developers.openai.com/codex/app/browser), [Computer Use](https://developers.openai.com/codex/app/computer-use)를 함께 들고 오면서 "ChatGPT 안의 Claude Code"에 더 가까운 표면이 생겼다. CLI 세션을 import하는 thin wrapper가 여전히 옳은 primitive인지, 아니면 SDK 레벨에서 thread를 직접 생성·재개하는 쪽이 더 자연스러운지 — codex-r의 SKILL.md 한 장이 다음 라운드에서 어디로 흡수될지의 신호다.
...
**Codex 표면 확장 (2026-05-04 전후)**
- [Codex in ChatGPT — OpenAI Help Center](https://help.openai.com/en/articles/11369540-codex-in-chatgpt) — ChatGPT 플랜으로 Codex 사용, 4개 진입점(앱·CLI·IDE·web) 정리
- [Codex Python SDK — `openai/codex/sdk/python`](https://github.com/openai/codex/tree/main/sdk/python) — `app-server` JSON-RPC v2 위의 실험적 Pydantic SDK (`openai-codex-app-server-sdk` + `openai-codex-cli-bin`)
- [Codex web (chatgpt.com/codex)](https://chatgpt.com/codex) — GitHub 연결 기반의 클라우드 Codex 진입점
- [Codex 개발자 문서 허브](https://developers.openai.com/codex/) — 모델·메모리·자동화·컴퓨터 사용 등 ChatGPT 통합 표면 일람
