...
```mermaid
graph TD
    Lead[Team Lead] -->|생성| T1[Teammate 1]
    Lead -->|생성| T2[Teammate 2]
    Lead -->|생성| T3[Teammate 3]
    T1 --- T2
    T2 --- T3
    T1 --- T3
...
```mermaid
graph LR
    MA[Main Agent] -->|지시| SA1[Subagent 1]
    MA -->|지시| SA2[Subagent 2]
    SA1 -->|결과| MA
    SA2 -->|결과| MA
```

**Agent Teams 모델**에서는 이 구조가 달라진다:

```mermaid
graph LR
    TL[Team Lead] -->|조율| AT1[Teammate 1]
    TL -->|조율| AT2[Teammate 2]
    AT1 --- AT2
...
```mermaid
graph TD
    A[Teammate A] -->|편집| FS[공유 파일시스템]
    B[Teammate B] -->|편집| FS
    FS --> CONFLICT[파일 충돌]
    C[Teammate A] -->|편집| WT1[Worktree A]
    D[Teammate B] -->|편집| WT2[Worktree B]
