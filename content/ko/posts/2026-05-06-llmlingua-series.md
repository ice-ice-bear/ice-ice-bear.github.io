...
한 토론에서 누군가 [LLMLingua](https://github.com/microsoft/LLMLingua)를 언급했고, 다른 사람이 *"네 굉장히 저평가 되있다고 생각합니다"* 라고 동의했다. 별 6,156개에 MIT 라이선스, EMNLP'23부터 CoLM 2025까지 6편의 논문이 이어진 시리즈인데도 운영 사례를 찾기 어려운 도구다. 압축률 20배에 거의 무손실이라는 강력한 결과가 있는데 왜 production 채택이 더디게 진행되는지 — "저평가"라는 이 한 단어를 풀어보면 **연구 → 프로덕션 사이의 갭**이 그대로 보인다.
